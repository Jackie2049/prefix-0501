...
class ThdBatchLayout:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cu_seqlens != self._cumsum(self.padded_lengths):
    ...
    def construct_from_valid_lengths(cls, valid_lengths: Sequence[int]) -> "ThdBatchLayout":
        ...
        return cls(
            ...
            cu_seqlens=cls._cumsum(lengths),
    ...
    def construct_from_kept_position_ids(
        ...
        *,
        ...
        return cls(
            ...
            cu_seqlens=cls._cumsum(padded_lengths),
    ...
    def has_padding(self) -> bool:
        """True when at least one row has alignment padding."""
    ...
    def unpad(self, tensor: Any) -> Any:
        """Strip alignment padding from a packed tensor, keeping only valid tokens."""
        if not self.has_padding:
            return tensor
        rows = list(torch.split(tensor, self.padded_lengths, dim=0))
        return torch.cat(
            [row[:valid_length] for row, valid_length in zip(rows, self.valid_lengths)],
            dim=0,
        )

    def repad(self, tensor: Any) -> Any:
        """Re-apply alignment padding to a valid-only packed tensor."""
        if not self.has_padding:
            return tensor
        rows = list(torch.split(tensor, self.valid_lengths, dim=0))
        repadded = []
        for row, padded_length, valid_length in zip(rows, self.padded_lengths, self.valid_lengths):
            if padded_length == valid_length:
                repadded.append(row)
                continue
            pad = torch.zeros(
    ...
    def _cumsum(lengths: Sequence[int]) -> list[int]:
        """Compute cumulative sum of lengths, starting with 0."""
        values = [0]
        running = 0
