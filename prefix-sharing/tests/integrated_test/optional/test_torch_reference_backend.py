...
def test_torch_reference_backend_supports_q_len_different_from_kv_len():
    plan = PrefixSharingPlanner(PrefixSharingConfig(enable_prefix_sharing=True, min_prefix_len=2)).plan(
        [[1, 2, 10], [1, 2, 20, 21]],
        forward_id=1,
        micro_batch_id=1,
    )
    ...
    store = PrefixKVStore()
    query = torch.randn(sum(plan.kept_lengths_q), 4)
    key = torch.randn(sum(plan.kept_lengths_q), 4)
    value = torch.randn(sum(plan.kept_lengths_q), 4)
...
def test_torch_reference_backend_supports_thd_grouped_query_attention():
    ...
    backend = TorchReferenceBackend()
    store = PrefixKVStore()
    query = torch.randn(sum(plan.kept_lengths_q), 4, 8)
    key = torch.randn(sum(plan.kept_lengths_q), 2, 8)
    value = torch.randn(sum(plan.kept_lengths_q), 2, 8)
...
def test_torch_reference_backend_caches_expanded_reuser_for_later_reuse():
    ...
    backend = TorchReferenceBackend()
    store = PrefixKVStore()
    key = torch.arange(sum(plan.kept_lengths_q) * 2, dtype=torch.float32).reshape(-1, 2)
    value = key + 100
    ...
    reuser_slot_id = PrefixKVSlotId(plan.forward_id, plan.micro_batch_id, 0, 1, 0)
...
def test_prefix_last_restore_tensor_keeps_autograd_path():
    ...
    restored_logits = gather_provider_prefix_last_logits(logits, plan)
...
def test_torch_reference_backend_cuda_optional_smoke():
    ...
    backend = TorchReferenceBackend()
    query = torch.randn(sum(plan.kept_lengths_q), 4, device="cuda")
    key = torch.randn(sum(plan.kept_lengths_q), 4, device="cuda")
    value = torch.randn(sum(plan.kept_lengths_q), 4, device="cuda")
    expanded_key, expanded_value = backend.build_kv(
        key,
        value,
        PrefixKVStore(),
    ...
    assert backend.attention(query, expanded_key, expanded_value, plan).is_cuda
