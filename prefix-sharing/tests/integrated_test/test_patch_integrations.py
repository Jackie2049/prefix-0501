...
def test_verl_mcore_batch_adapter_uses_mapping_for_preprocess_and_restore():
    ...
    assert [(s.reuse_idx_in_batch, s.provider_idx_in_batch, s.prefix_len) for s in prepared.prefix_sharing_plan.reuse_specs] == [
        (1, 0, 3),
    ]
    ...
    with adapter.prepared_context(prepared) as ctx:
        ...
        assert ctx.prefix_sharing_plan is prepared.prefix_sharing_plan
