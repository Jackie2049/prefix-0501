"""Tests for PackedBatchLayout validation, unpad/repad, and edge cases.

Focuses on the core business logic paths that were uncovered: __post_init__
validation guards, unpad/repad for TP padding, has_padding, valid_slice,
and from_kept_position_rows edge cases.
"""
...
# ------------------------------------------------------------------
# from_kept_position_rows edge cases
# ------------------------------------------------------------------


def test_from_kept_position_rows_empty_returns_empty_layout():
    layout = PackedBatchLayout.from_kept_position_rows([], align_size=4)
...
def test_from_kept_position_rows_align_size_zero_raises():
    with pytest.raises(ValueError, match="align_size must be >= 1"):
        PackedBatchLayout.from_kept_position_rows(
...
def test_has_padding_false_when_no_padding():
    layout = PackedBatchLayout.from_valid_lengths([4, 3, 5])
...
def test_valid_slice_returns_valid_tokens():
    layout = PackedBatchLayout.from_valid_lengths([5, 3])
    tensor = torch.arange(8)  # [0,1,2,3,4,5,6,7]
    # Row 0: valid slice should be indices 0..4 → [0,1,2,3,4]
    assert torch.equal(layout.valid_slice(0, tensor), tensor[:5])
    # Row 1: valid slice should be indices 5..7 → [5,6,7]
    assert torch.equal(layout.valid_slice(1, tensor), tensor[5:8])


def test_valid_slice_with_padding_layout():
    rows = [torch.zeros(5, dtype=torch.long), torch.zeros(3, dtype=torch.long)]
    layout = PackedBatchLayout.from_kept_position_rows(rows, align_size=4)
    ...
    assert torch.equal(layout.valid_slice(1, tensor), tensor[8:11])
...
def test_total_valid_length():
    layout = PackedBatchLayout.from_valid_lengths([5, 3, 7])
