...
async def estimate_with_mbridge(config: MBridgeEstimateConfig):
    ...
    if hasattr(hf_config, "vocab_size"):
        ...
        raise ValueError(
            f"Vocab size attribute not found for the current model configuration {type(hf_config).__name__}. "
    ...
    if hasattr(hf_config, "max_position_embeddings"):
        ...
        raise ValueError(
            f"Max position embeddings attribute not found for the current model configuration {type(hf_config).__name__}. "
