...
class MegatronPPOActor(BasePPOActor):
    ...
    def update_policy(self, dataloader: Iterable[DataProto]) -> dict:
        ...
        if profile_output_dir:
            from prefix_sharing.tools.training_monitor import MemoryMonitor, PerfRecorder
            from megatron.core.pipeline_parallel.schedules import _profile_stopwatch_var

            _profile_ctx_var = _profile_stopwatch_var
            self._profile_recorder = PerfRecorder(enabled=True, rank=int(os.getenv("RANK", os.getenv("LOCAL_RANK", "0"))))
        ...
        for data in dataloader:
            ######### profiling #########
            if profile_output_dir:
                self._profile_recorder.set_step_context(_train_step)
                _profile_ctx_var.set(self._profile_recorder)
            ######### profiling #########
            ...
            if profile_output_dir:
                ...
                self._profile_recorder.start("update")
            ######### profiling #########
            ...
            ######### profiling #########
            if profile_output_dir:
                self._profile_recorder.stop("update")
                _train_step += 1
            ######### profiling #########
        ...
        if profile_output_dir:
            ...
            self._profile_recorder.save_to_csv(os.path.join(profile_output_dir, f"timing_trace_rank{rank}.csv"))
            self._profile_recorder.flush()
            logger.warning("[PS][perf] profiling summary:\n%s",
                           json.dumps(self._profile_recorder.summary(), indent=2))
            logger.warning("[PS][perf] memory summary:\n%s",
                           json.dumps(self._profile_mon.summary(), indent=2))
