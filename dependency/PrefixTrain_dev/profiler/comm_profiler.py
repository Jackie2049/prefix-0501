...
def parse_args():
    ...
    parser.add_argument('--prof-cp-size', type=int, default=None, help='Profiler cp size.')
    ...
    parser.add_argument('--prof-seq-len-list', nargs='+', type=int, default=None, help='')
...
def run(rank, world_size, data_size_list, model, size, torch_data_type):
    ...
    dist.init_process_group(backend='hccl', world_size=world_size, rank=rank, init_method=init_method)
                    ...
                    try:
                        if not args.prof_skip_running:
                            if collective_type == "all_gather":
                                send_tensor = torch.ones(data_size, dtype=torch_data_type).npu()
                                tensor_list = [torch.zeros(data_size, dtype=torch_data_type).npu() for _ in range(world_size)]
                                ...
                                if data_size % world_size == 0:
                                    ...
                                    send_tensor = torch.ones(_data_size, dtype=torch_data_type).npu()
                                for i in range(args.prof_repeat_times + args.prof_warmup_times): 
                                    torch.npu.synchronize()
                                    start = time.time()          
        ...
        for tp in tp_size_list:
            for cp in cp_size_list:
                for algo in algo_list:
                    file_name = args.prof_op_time_path + f"{model}_{size}_seq{}_mbs{mbs}_tp{tp}_cp{cp}_algo{algo}.csv"
