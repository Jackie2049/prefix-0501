...
from megatron.model import DistributedDataParallel_Profile as LocalDDP_Profile
from megatron.utils import unwrap_model
from megatron.model import Float16Module
from megatron.utils import debug_mem_report
from megatron.utils import report_memory
from model_configs import model_prof_configs, resnet_configs, gpt_configs, t5_configs , llama2_config
...
def wrap_op(op):
    ...
    input_mats = np.array(all_ranks).reshape([1, 1, 1, op.dp_size, op.tp_size * op.cp_size])    #TODO if use cp will not affect
    input_mats_ = {}
...
def get_model(model_name, model_size,seq_len_input = None):
    ...
    if model_name == "resnet":
        ...
        params_dtype = get_params_dtype(params_dtype)
        if(seq_len_input !=None):
            args.seq_length = seq_len_input
        else:
...
def infer_data_size(op_list, save_filename_prefix, mbs, algo,seq_len):
    '''
    ...
    cp_size = args.prof_cp_size

    prev_extra_size = 0
    for op_info in op_list:
        op_info["op_index"] = 0
        op_uniq_name = save_filename_prefix + op_info["name"] + f"seq_len{seq_len}mbs{mbs}tp_size{tp_size}cp_size{cp_size}algo{algo}"
        op = unwrap_model(gen_op(op_info, algo=algo), (LocalDDP_Profile, Float16Module)) 
        ...
        for input_name in op.input_tensors_info:
            ...
            try:
                ...
                print("input_tensors_info",input_name)
                exit()
            ...
            if(cp_split_dim!=-1):
                input_shape[cp_split_dim] //= op.cp_size
        ...
        for output_name in op.output_tensors_info:
            ...
            try:
                ...
                print("output_tensors_info",output_name)
                exit()
            ...
            if(cp_split_dim!=-1):
                output_shape[cp_split_dim] //= op.cp_size
        ...
        for input_extra_name in op.input_extra_tensors_info:
            ...
            try:
                ...
                print("input_extra_tensors_info",op,op.input_extra_tensors_info,input_extra_name)
                exit()
        ...
        for output_extra_name in op.output_extra_tensors_info:
            ...
            try:
                cp_split_dim = op.output_extra_tensors_info[output_extra_name]["cp_split_dim"]
            except:
                print("output_extra_tensors_info",output_extra_name)
                exit()
        ...
        current_extra_size = prev_extra_size + sum_output_extra_size - sum_input_extra_size # prev_extra_size 指类似残差连接的情况
        print(f"{op_info['name']}: current_extra_size = {current_extra_size}= prev_extra_size {prev_extra_size}+ sum_output_extra_size {sum_output_extra_size}- sum_input_extra_size {sum_input_extra_size} sum_input_size {sum_input_size}")
...
def get_op_hash(op_info, seq_len,micro_batch_size, tp_size, cp_size, algo, save_filename_prefix):
    hash_str = save_filename_prefix + "seq_len"+ str(seq_len) + "mbs" + str(micro_batch_size) + "tp" + str(tp_size) + "cp" + str(cp_size)
    args = get_args()
...
def get_outputs_and_grads(output_tensors, output_extra_tensors, grad_type):
    ...
    for output_tensor in origin_outputs:
        ...
        if len(tensor_shape) >= 3:
            ...
            pool_op = torch.nn.AdaptiveAvgPool2d(1)
        elif len(tensor_shape) >= 2:
            # pool_op = torch.nn.AdaptiveMaxPool1d(1)
            pool_op = torch.nn.AdaptiveAvgPool1d(1)
        else:
...
def profile_op(mbs, algo, op_info, params_dtype, grad_type, op_uniq_name): #TODO: 为什么mbs没有用到
    ...
    if op_info["name"] in ["gpt-post-process", "t5-post-process"]:
        ...
        tensor_value = tensor_value / torch.distributed.get_world_size()
        ...
        if op_info["name"] == "encoder-embedding" or  op_info["name"] == "encoder-embedding-v1" :
            ...
            torch.autograd.backward(origin_outputs, output_grads,  retain_graph=True)
...
def dump_profiled_results(save_filename_prefix, seq_len,mbs, algo, op_list):
    global profiled_results
    args = get_args()
    if torch.distributed.get_rank() == 0:
        print_rank0(f"====== PROFILING RESULTS ({save_filename_prefix}, mbs = {mbs}, tp = {args.prof_tp_size}, algo = {algo}) ======")
        save_file_name = f"{save_filename_prefix}_seq{seq_len}_mbs{mbs}_tp{args.prof_tp_size}_cp{args.prof_cp_size}_algo{algo}.csv"
        ...
        with open(args.prof_path + save_file_name,'w')as f_result:
            f_csv = csv.writer(f_result)
            f_csv.writerow(result_title)
            for op_info in op_list:
                op_name = save_filename_prefix + op_info["name"] + f"seq_len{seq_len}mbs{mbs}tp_size{args.prof_tp_size}cp_size{args.prof_cp_size}algo{algo}"
...
def run_profile(task):
    ...
    op_list = get_model(model, size,seq_len).full_op_list
    ...
    save_filename_prefix = f"{model}_{size}"
    print("op_hash_list",op_hash_list)
    args.micro_batch_size = mbs
    for algo in algo_list:
        ## infer the data size according to op specs
        infer_data_size(op_list, save_filename_prefix, mbs, algo,seq_len)
        # run profiling
        for op_info in op_list:
            op_uniq_name = save_filename_prefix + op_info["name"] + f"seq_len{seq_len}mbs{mbs}tp_size{tp_size}cp_size{cp_size}algo{algo}" 
            op_hash = get_op_hash(op_info, seq_len, mbs, tp_size, cp_size, algo, save_filename_prefix)
            if op_uniq_name in profiled_results:
                print_rank0(f"working on {op_info['name']}, seq = {seq_len}, mbs = {mbs}, tp = {tp_size}, algo = {algo} ... Hit same op in cache!!!")  
                continue
            elif op_hash in op_hash_list:
                print_rank0(f"working on {op_info['name']}, seq = {seq_len}, mbs = {mbs}, tp = {tp_size}, algo = {algo} op_hash = {op_hash} ... Hit identical op in cache!!!")  
                ...
                print_rank0(f"working on {op_info['name']}, seq = {seq_len}, mbs = {mbs}, tp = {tp_size}, algo = {algo} ... ")  
                try:
                    if SKIP_RUNNING:
        ...
        dump_profiled_results(save_filename_prefix,seq_len, mbs, algo, op_list)
...
if __name__ == "__main__":
    ...
    if args.prof_cache_file!=None and os.path.exists(args.prof_cache_file):
        print(f"read args.prof_cache_file {args.prof_cache_file}")
    ...
    for model in model_names:
        ...
        for seq_len in args.prof_seq_len_list:
            for size in model_sizes:
                ...
                for mbs in micro_batch_sizes:
                    all_prof_tasks.append({"model": model, "size": size, "mbs": mbs , "seq_len":seq_len})
