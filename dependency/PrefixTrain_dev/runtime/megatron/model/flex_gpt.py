...
def _gpt_v1(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, hidden_dropout, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels):
    op_list = []
    op_list.append({"name": "encoder-embedding-v1", "op_index": 0, "embedding_dropout_prob": hidden_dropout, "init_method": init_method, "num_tokentypes": num_tokentypes, "prev_name": None})
    prev_name = "encoder-embedding-v1"
    for i in range(num_layers):
        op_list.append({"name": "enc-1st-layernorm-v1", "op_index": 1 + i * 10 + 0, "prev_name": prev_name})
        op_list.append({"name": "enc-attention-qkv-v1", "op_index": 1 + i * 10 + 1, "init_method": init_method, "attention_type": AttnType.self_attn, "prev_name": "enc-1st-layernorm"})
        op_list.append({"name": "enc-flash-attention", "op_index": 1 + i * 10 + 2, "layer_number": i, "prev_name": "enc-attention-qkv"})
        op_list.append({"name": "enc-attention-dense", "op_index": 1 + i * 10 + 3, "init_method": output_layer_init_method, "dim1_size": kv_channels * num_attention_heads, "dim2_size": hidden_size, "prev_name": "enc-flash-attention"})
        op_list.append({"name": "enc-post-attention-dropout-v1", "op_index": 1 + i * 10 + 4, "prev_name": "enc-attention-dense"})
        op_list.append({"name": "enc-2nd-layernorm-v1", "op_index": 1 + i * 10 + 5, "prev_name": "enc-post-attention-dropout"})
        op_list.append({"name": "enc-MLP-GEMM-1", "op_index": 1 + i * 10 + 6, "init_method": init_method, "dim1_size": hidden_size, "dim2_size": ffn_hidden_size, "prev_name": "enc-2nd-layernorm"})
        op_list.append({"name": "enc-MLP-gelu", "op_index": 1 + i * 10 + 7, "prev_name": "enc-MLP-GEMM-1"})
        op_list.append({"name": "enc-MLP-GEMM-2", "op_index": 1 + i * 10 + 8, "init_method": output_layer_init_method, "dim1_size": ffn_hidden_size, "dim2_size": hidden_size, "prev_name": "enc-MLP-gelu"})
        op_list.append({"name": "enc-post-MLP-dropout", "op_index": 1 + i * 10 + 9, "prev_name": "enc-MLP-GEMM-2"})
        prev_name = "enc-post-MLP-dropout"

    op_list.append({"name": "final-layernorm", "op_index": 1 + num_layers * 10 + 0, "prev_name": prev_name})
    op_list.append({"name": "gpt-post-process", "op_index": 1 + num_layers * 10 + 1, "parallel_output": parallel_output, "init_method": init_method, "prev_name": "final-layernorm"})
    ...
    def __init__(self,
        ...
        if not profiling:
            ...
            if(args.prof_new_gpt):
                full_op_list = _gpt_v1(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, \
            ...
            self.language_model = get_flex_model( #在这调用了实现recompute的forward函数
            ...
            output_layer_init_method = scaled_init_method_normal(args.init_method_std, num_layers)
            if(args.prof_new_gpt):
                self.full_op_list = _gpt_v1(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, \
