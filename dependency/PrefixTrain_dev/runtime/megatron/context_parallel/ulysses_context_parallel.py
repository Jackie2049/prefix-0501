...
class UlyssesContextAttention_kvpack(torch.nn.Module):
    """Initialization.
    Arguments:
        local_attention (Module): local attention with q,k,v
        sequence_process_group (ProcessGroup): sequence parallel process group
    ...
    def __init__(
        ...
        super(UlyssesContextAttention_kvpack, self).__init__()
        self.spg = sequence_process_group
        self.scatter_idx = scatter_idx
        self.gather_idx = gather_idx
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, *args: Any, **kwargs: Any) -> Tensor:
        ...
        packed_seq_len, num_head = query_layer.shape[0], query_layer.shape[2]
        query_layer = query_layer.view(packed_seq_len, num_head, -1)

        key_vaule_pack = torch.cat([key_layer, value_layer], dim=1)
        # out shape : e.g., [s:h/p:]
        context_layer = flash_attn_varlen_kvpacked_func(query_layer, key_vaule_pack, *args, **kwargs)
