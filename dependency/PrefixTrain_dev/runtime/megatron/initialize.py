...
def _initialize_distributed():
    ...
    start_time_tensor = torch.cuda.DoubleTensor([1])
    if(args.distributed_backend == "gloo"):
        start_time_tensor = start_time_tensor.cpu()
    torch.distributed.all_reduce(start_time_tensor,
                                 op=torch.distributed.ReduceOp.MIN)
    # print(f'Rank {args.rank} : gloabl rank ({torch.distributed.get_rank()}) has data {start_time_tensor}')
    ...
    if device_count > 0:
        if mpu.model_parallel_is_initialized():
            ...
            if(args.prof_tp_size is None):
                mpu.initialize_hete_dp(args)
