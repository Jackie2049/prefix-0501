...
def get_op_list(args):
    global op_list
    gpu_name = args.gpu_type_num_dict["0"]["GPU"]
    if op_list is None:
        op_list = []
        if args.model_name == "scale-layer":
            ...
            if args.model_name == "t5" and args.model_size == "22B":
                model_size = "11B"
            src_data_file = args.profiled_time_path + gpu_name+ f"/{args.model_name}_{model_size}/"+ args.model_name + f"_{model_size}_seq{args.seq_len}_mbs{args.micro_batch_size[0]}_tp1_cp1_algo0.csv"
...
def get_full_op_list(args):
    ...
    if full_op_list is None:
        if args.model_name in ["gpt", "scale-layer"]:
            head_ops = [op_list[0]]
            decoder_layer = op_list[1:args.num_ops_each_layer+1]
            tail_ops = op_list[args.num_ops_each_layer+1:]
