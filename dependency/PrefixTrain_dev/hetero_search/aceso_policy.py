...
def get_target_stage(config, metric=None, other_info="", adaptive_flag=False):
    '''
    先决定要优化的metric，然后根据metric来决定要优化的stage
    '''
    ...
    for i in range(config.num_stages):
        min_memory = get_memory_limit(aceso_var.args,config.stages[i].node_id_num)
        if metric is None and max(config.memory_list) > min_memory:
    ...
    max_value = 0
    for i in range(config.num_stages):
        # 如果是adaptive_flag，那么只有当这个stage没有被访问过时才会被选中 而且这个stage的value要大于max_value
...
def get_actions_with_policy(config, bottleneck, action_resource_table, adaptive_flag=False):
    ...
    stage_type = ""
    # If memory is the bottleneck, we need to consider the memory limit.
    # if max(config.memory_list) > args.memory_limit:
    if check_memory_legacy(aceso_var.args,config)==False:
        _actions = get_actions_by_filters(action_resource_table, memory_choice=["-"])
        actions.append(_actions)    
        stage_type = "OOM"
    else:# If memory is not the bottleneck, we can consider the time, comm, and memory.
        comp_time = config.breakdown_ideal_time_per_gpu[bottleneck] # ideal time for the bottleneck stage
        avg_comp_time = sum(config.breakdown_ideal_time_per_gpu)/config.num_stages # average ideal time per stage
        if comp_time > 0: # if the ideal time is greater than 0 then calculate the ratio
            comp_time_ratio = config.breakdown_ideal_time_per_gpu[bottleneck] / sum(config.breakdown_ideal_time_per_gpu)
        else:
            comp_time_ratio = 0
        eff_loss_time = config.breakdown_eff_loss_time_per_gpu[bottleneck]# eff_loss time for the bottleneck stage
        ...
        recomp_time = config.breakdown_recomp_time_per_gpu[bottleneck]# recomp time for the bottleneck stage
        ...
        visited_actions = []
        while index < 3:
            ## - comp_time 如果ideal_time是最大的，那么就考虑减少ideal_time
            if comp_time_ratio == max(tmp_list):
                if eff_loss_time > avg_eff_loss_time:# '-' ,'0'表示减少通信或者不变
                    comm_choice = ["-", "0"]
                else:
                    comm_choice = ["+", "-", "0"]
                if recomp_time > 0: # 如果recomp_time大于0，那么就考虑减少recomp_time
                    memory_choice = ["-", "0"]
                else:
                    memory_choice = ["+", "-", "0"]
                ...
                if index == 0:
                    NUM_CASES_HIT[0] += 1
            ## - comm_time # 如果eff_loss_time是最大的，那么就考虑减少eff_loss_time
...
def get_actions(config, bottleneck, action_resource_table, adaptive_flag):
    if aceso_var.args.random_order_actions:
