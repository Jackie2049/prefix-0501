...
CONFIG_SAVE_PATH=$ROOT_PATH/tools/configs/test/
mkdir -p ${LOG_PATH}csv
export DEBUG_COMMUNICATE=1
for file_name in $(ls $CONFIG_SAVE_PATH)
do

if [ "$file_name" != "dp_brief_v1.json" ]; then
    continue
fi
...
NCCL_SOCKET_IFNAME=eno12399np0 torchrun $DISTRIBUTED_ARGS \
       pretrain_gpt.py \
       ...
       --mbs-seq '[[[4,1024],[4,1024],[4,1024],[4,1024]]*3,[[2,1024],[4,1024],[4,1024]]*18]'
    #    --log-detail-compute-time \
