...
if [ "$exp_setting" == "small" ]; then
    ...
    #### Paths ####
    RESULT_PATH=${ROOT_PATH}/logs/our_test/
    ...
    ## 2nodes
    cd $ROOT_PATH/runtime && bash scripts/run_${model_name}_2nodes_ours.sh
    # parallel-ssh -i -t 0 -h pssh-2nodes.host "ps -aux | grep 'pretrain' | grep -v grep | awk '{print \$2}' | xargs -r kill -9"
    # parallel-ssh -i -t 0 -h pssh-2nodes.host "cd $ROOT_PATH/runtime && bash scripts/run_${model_name}_2nodes_ours.sh"
