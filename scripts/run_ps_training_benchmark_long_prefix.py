#!/usr/bin/env python3
"""PS training benchmark with realistic GRPO prefix lengths.

Previous benchmarks used prefix=64, suffix=64 which showed PS ON slower (0.69-0.76x)
due to kernel launch overhead in the prefix pass (~110ms fixed cost).

This version tests prefix=256, suffix=64 (75% prefix ratio) which is more realistic
for GRPO training (long prompts, short responses). At this ratio, the prefix pass
cost should be a smaller fraction of total training time, making PS ON faster.

Usage: torchrun --nproc_per_node=4 scripts/run_ps_training_benchmark_long_prefix.py
"""
...
PREFIX_LEN = 256
SUFFIX_LEN = 64
N_SEQUENCES = 4  # n=4 with 256+64=320 total, should fit in 24GB
SEED = 42
NUM_WARMUP = 1
NUM_STEPS = 5
LR = 1e-5
...
if local_rank == 0:
    ...
    print(f"Config: n={N_SEQUENCES}, prefix={PREFIX_LEN}, suffix={SUFFIX_LEN}, "
          f"token_savings={token_savings:.1%}")
...
# ===== PS ON forward (from n-sweep, adapted for PREFIX_LEN=256) =====
def ps_on_forward(n_val, suffix_ids_lists):
...
with torch.no_grad():
    logits_ps = ps_on_forward(N_SEQUENCES, suffix_ids_prec)
...
if local_rank == 0:
    ...
    print(f"  Theoretical speedup: {theoretical:.2f}x")
    print(f"{'='*60}")

# --- PS OFF model.forward() ---
if local_rank == 0:
    print("\n[A] PS OFF model.forward()")
...
for step in range(NUM_WARMUP + NUM_STEPS):
    ...
    torch.cuda.synchronize()
    t0 = time.time()
    logits = ps_on_forward(N_SEQUENCES, suffix_lists)
    shift_l = logits[:, :-1, :]
...
if local_rank == 0:
    ...
    print(f"  n={N_SEQUENCES}, prefix={PREFIX_LEN}, suffix={SUFFIX_LEN}")
    print(f"  Token savings: {token_savings:.1%}")
    print(f"  Theoretical: {theoretical:.2f}x")
    ...
    result = {
        "n": N_SEQUENCES, "prefix_len": PREFIX_LEN, "suffix_len": SUFFIX_LEN,
        "token_savings_pct": token_savings * 100, "theoretical_speedup": theoretical,
        "off_avg_s": avg_off, "on_avg_s": avg_on, "speedup": speedup,
        "peak_off_gb": peak_off, "peak_on_gb": peak_on, "precision_cos_sim": overall_cos,
    }
    result_file = os.path.expanduser("~/rollout-prefix/ps_long_prefix_results.json")
