#!/usr/bin/env python3
"""End-to-end test for two-pass PS forward helper with pretrained weights.
...
using pretrained Qwen3-27B-text-only weights.

Usage: torchrun --nproc_per_node=4 scripts/test_two_pass_forward_pretrained.py
"""
import os
import sys
import time
...
# ===== Load pretrained weights (same logic as run_ps_e2e_twopass_v2.py) =====
if local_rank == 0:
...
if local_rank == 0:
    print(f"\n{'='*60}")
    print(f"TWO-PASS PS FORWARD HELPER E2E TEST (PRETRAINED)")
    print(f"{'='*60}")
    print(f"Model: Qwen3.6-27B (TP=4, bf16, pretrained weights)")
