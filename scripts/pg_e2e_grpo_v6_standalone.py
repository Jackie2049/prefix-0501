#!/usr/bin/env python3
"""
PG E2E GRPO Competitive Analysis — Enhanced Standalone Script v6
Uses verl's PrefixGrouper monkey-patch for PG ON (proper prefix_grouper support).
Collects per-step timing: rollout, logprob, advantages, train.
Both PG OFF (normal) and PG ON (PrefixGrouper) are supported.
"""
...
BATCH_SIZE = int(os.environ.get("BATCH_SIZE", "4")) # prompts per step
SEED       = 42
...
def compute_log_probs_normal(model, input_ids, attention_mask, prompt_lens):
    ...
    for i in range(input_ids.shape[0]):
        ...
        # Slice to suffix positions before log_softmax (prevent OOM)
...
def compute_log_probs_pg(model, input_ids, attention_mask, prompt_lens, uids, tokenizer):
    ...
    # Build response mask and responses tensors
    response_mask = torch.zeros_like(attention_mask)
    for i in range(batch_size):
        response_mask[i, prompt_lens[i]:] = attention_mask[i, prompt_lens[i]:]

    # Extract prompt tokens (prefix) and response tokens
    prompts_tensor = input_ids[:, :P_LEN]  # approximate prompt length
    responses_tensor = input_ids  # use full input_ids as "responses" (prefix + response)

    # Build micro_batch dict for PG
    micro_batch = {
        "prompts": input_ids,  # full sequence acts as "prompts" for PG grouping
        "responses": input_ids,  # same
        "response_mask": response_mask,
        "uid": uids,
        "pad_token_id": pad_token_id,
    }

    # Build PG from micro_batch
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        log_probs_pg, _, suffix_mask = pg_forward(
            ...
            completion_ids=input_ids,  # original input_ids as completion
...
def main():
    ...
    print(flush=True)
    print("=== PG E2E GRPO Competitive Analysis v6 (Enhanced) ===", flush=True)
    ...
    for step in range(NUM_STEPS):
        ...
        # Ref model log probs (always normal)
        with torch.no_grad():
    ...
    # Summary
    avg_step = sum(r['step_time'] for r in results[1:]) / max(1, len(results)-1)
    ...
    output_file = f"/home/zxw/rollout-prefix/pg_e2e_v6_results_{'pg_on' if USE_PG else 'pg_off'}.json"
