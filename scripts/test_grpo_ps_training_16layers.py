#!/usr/bin/env python3
"""GRPO PS training test with 16-layer reduced model.

Uses Qwen3-27B-text-only-16layers (4 full attn + 12 DeltaNet = ~7B params)
to fit backward pass on 24GB GPU with gradient checkpointing.

Validates:
1. PS precision alignment (log_prob cos_sim > 0.99)
2. PPO clipped policy loss computation
3. Backward pass and gradient flow through PS patches

Usage: torchrun --nproc_per_node=4 scripts/test_grpo_ps_training_16layers.py
"""
import os, sys, time
...
if local_rank == 0:
    print(f"\n=== GRPO PS Training (16-layer, N={N_SEQUENCES}, P={PREFIX_LEN}, S={SUFFIX_LEN}) ===")
...
if local_rank == 0:
    print(f"Step 1 done, GPU {torch.cuda.memory_allocated()/1024**3:.2f} GB")

# Step 2: Two-pass PS forward (with gradients)
...
torch.cuda.synchronize(); t_ps_start = time.time()

with prefix_sharing_runtime_context(ps_runtime_state) as ctx_state:
    # Prefix pass (no grad needed)
    with torch.no_grad():
...
if local_rank == 0:
    print(f"Precision: log_prob cos_sim={overall_cos:.6f} [{('PASS' if overall_cos>=0.99 else 'FAIL')}]")

# Step 4: GRPO advantage
...
policy_loss = policy_loss_per_pos.mean()

if local_rank == 0:
    print(f"Policy loss: {policy_loss.item():.4f}, Ratio mean: {ratio.mean().item():.4f}")
...
try:
    ...
    if local_rank == 0:
        print(f"Backward PASS: grad_norm={total_grad_norm:.4f}, "
              f"params_with_grad={params_with_grad}, layers_with_grad={n_layers_grad}/{num_layers}")
        grad_status = "PASS" if n_layers_grad == num_layers else "PARTIAL"

except torch.cuda.OutOfMemoryError:
    if local_rank == 0:
        print("Backward OOM — try fewer sequences or gradient checkpointing")
    model.zero_grad(); torch.cuda.empty_cache()
    grad_status = "OOM"; total_grad_norm = -1.0; n_layers_grad = 0

patch_handle.disable()

if local_rank == 0:
    print(f"\n{'='*60}")
    print(f"GRPO PS TRAINING (16-layer ~7B)")
    print(f"{'='*60}")
    print(f"Precision: cos_sim={overall_cos:.6f}")
    print(f"Policy loss: {policy_loss.item():.4f}")
    print(f"Gradient: {grad_status}, norm={total_grad_norm:.4f}")
    print(f"PS time: {t_ps:.3f}s")
    print(f"{'='*60}")
    if overall_cos >= 0.99 and grad_status == "PASS":
        print("PASS: GRPO PS training with backward!")
    elif overall_cos >= 0.99:
        print(f"PARTIAL: Forward PASS, Backward {grad_status}")
