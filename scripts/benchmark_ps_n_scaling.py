#!/usr/bin/env python3
"""Comprehensive PS scaling benchmark: varying N (number of sequences sharing prefix).

Tests two-pass PS with N=4, 8, 16, 32 to find when PS becomes faster than normal forward.
Also measures precision alignment (cos_sim) for each N.

Usage: torchrun --nproc_per_node=4 scripts/benchmark_ps_n_scaling.py
"""
import os, sys, time
...
TP_SIZE = 4; PREFIX_LEN = 64; SUFFIX_LEN = 32; SEED = 42; N_WARMUP = 2; N_MEASURE = 5
...
# Install PS patches (installed once, reused for all N)
...
if local_rank == 0:
    print(f"\n{'='*80}")
    print(f"PS SCALING BENCHMARK: P={PREFIX_LEN}, S={SUFFIX_LEN}")
    print(f"{'='*80}")
    print(f"{'N':>4} {'Normal_t':>8} {'PS_t':>8} {'Ratio':>8} {'Savings':>8} {'cos_sim':>8}")
    print(f"{'-'*4} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")

results = []
for N in [4, 8, 16, 32]:
    torch.manual_seed(SEED + 500 + N)
    ...
    for i in range(N_MEASURE):
        ...
        with prefix_sharing_runtime_context(ps_runtime_state) as ctx_state:
            ...
            logits_ps = suffix_output.logits.clone()
        torch.cuda.synchronize()
    ...
    for j in range(N):
        cos_sim = F.cosine_similarity(
            ps_suffix_logits[j].float().flatten(),
            normal_suffix_logits[j].float().flatten(),
            dim=0,
        ).item()
    ...
    # Compute savings
    normal_tokens = N * total_len
    ps_tokens = PREFIX_LEN + N * SUFFIX_LEN
    savings_pct = (normal_tokens - ps_tokens) / normal_tokens * 100
    speedup_ratio = t_normal / t_ps

    if local_rank == 0:
        print(f"{N:>4} {t_normal:>8.3f}s {t_ps:>8.3f}s {speedup_ratio:>8.2f}x {savings_pct:>7.1f}% {overall_cos:>8.6f}")

    results.append((N, t_normal, t_ps, speedup_ratio, savings_pct, overall_cos))

    # Free memory
    del logits_normal, logits_ps, full_input_ids, suffix_input_ids, prefix_input_ids
    torch.cuda.empty_cache()
...
if local_rank == 0:
    ...
    for n, tn, tp, ratio, sav, cos in results:
        status = "PASS" if cos >= 0.999 else "FAIL"
        faster = "PS faster" if ratio > 1.0 else "Normal faster"
        print(f"  N={n}: {faster} ({ratio:.2f}x), savings={sav:.1f}%, cos_sim={cos:.6f} [{status}]")

    # Estimate crossover N
    if len(results) >= 2:
        # Linear extrapolation from the last two data points
        n1, t_n1, t_p1, *_ = results[-2]
        n2, t_n2, t_p2, *_ = results[-1]
        # t_normal ~ a * N * total_len / throughput
        # t_ps ~ prefix_const + b * N * suffix_len / throughput
        # We want t_normal = t_ps
        # Simplistic: ratio increases with N, extrapolate
        ratio1 = t_n1 / t_p1
        ratio2 = t_n2 / t_p2
        if ratio2 > ratio1 and ratio2 < 1.0:
            # Extrapolate when ratio reaches 1.0
            # ratio(N) ≈ ratio1 + (ratio2 - ratio1) * (N - n1) / (n2 - n1)
            # ratio = 1.0 → N ≈ n1 + (1.0 - ratio1) * (n2 - n1) / (ratio2 - ratio1)
            estimated_n = n1 + (1.0 - ratio1) * (n2 - n1) / (ratio2 - ratio1)
            print(f"\nEstimated crossover N ≈ {estimated_n:.0f} (where PS becomes faster than normal)")
        elif ratio2 >= 1.0:
            print(f"\nPS is already faster at N={n2}!")
