#!/usr/bin/env python3
"""
PG E2E GRPO Competitive Analysis v8 — Fair Comparison.

Key improvement over v7:
1. Both PG ON and PG OFF use flash_attention_2 (same attn_impl)
2. PG OFF does NOT apply monkey-patch (no generation overhead)
3. Also runs PG OFF with sdpa to show unfairness of v7 comparison
4. More configs: N=4,8; P=64,128,256
5. Proper timing breakdown: rollout, logprob, train

This is a competitive analysis of PR #4368 (PrefixGrouper).
...
USE_FLASH  = os.environ.get("USE_FLASH", "1") == "1"  # 1=flash_attention_2, 0=sdpa
...
# ── PG monkey-patch (only if USE_PG) ──
pg_available = False
if USE_PG:
    ...
    try:
        ...
        from verl.trainer.ppo.prefix_grouper_utils import (
            build_pg_from_micro_batch, pg_forward,
        )
        pg_available = True
        print("[PG ON] PrefixGrouper monkey-patch applied", flush=True)
    except Exception as e:
        print(f"[PG ON] Failed to load PG: {e}", flush=True)
        USE_PG = False

# ── Attention implementation ──
attn_impl = "flash_attention_2" if USE_FLASH else "sdpa"
if USE_PG:
    attn_impl = "flash_attention_2"  # PG requires flash_attention_2
...
def compute_log_probs_normal(model, input_ids, attention_mask, prompt_lens):
    """Standard forward — no prefix sharing. No no_grad since we need gradients for training."""
...
def main():
    ...
    mode_str = f"PG={'ON' if USE_PG else 'OFF'} attn={attn_impl}"
    print(f"\n=== PG E2E GRPO v8 — {mode_str} ===", flush=True)
    print(f"N={N}, BATCH={BATCH_SIZE}, P={P_LEN}, R={R_LEN}, STEPS={NUM_STEPS}", flush=True)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_PATH, dtype=torch.bfloat16, attn_implementation=attn_impl,
    ).to(DEVICE)
    model.train()
    ...
    for step in range(NUM_STEPS):
        ...
        print(f"  Rollout: {rollout_time:.3f}s ({n_seqs} seqs)", flush=True)

        # Phase 2: Log probs (actor + ref)
        logprob_t0 = time.time()

        if USE_PG and pg_available:
            log_probs = compute_log_probs_pg(
                model, input_ids, attn_mask, all_prompt_lens, all_uids, tokenizer
            )
        else:
            log_probs = compute_log_probs_normal(
                model, input_ids, attn_mask, all_prompt_lens
            )

        # Ref model log probs (always normal, no PG)
        with torch.no_grad():
        ...
        print(f"  LogProb: {logprob_time:.3f}s (PG={'yes' if USE_PG else 'no'})", flush=True)

        # Phase 3: GRPO advantages
        adv_t0 = time.time()
        advantages = torch.zeros_like(log_probs)
        for uid_val in set(all_uids):
            indices = [i for i, u in enumerate(all_uids) if u == uid_val]
        ...
        print(f"  Adv: {adv_time:.3f}s | Train: {train_time:.3f}s | Step: {step_time:.3f}s", flush=True)
        print(f"  Breakdown: rollout={rollout_time/step_time*100:.0f}% logprob={logprob_time/step_time*100:.0f}% train={train_time/step_time*100:.0f}%", flush=True)
        print(f"  Loss: {loss.item():.6f} | Mem: {peak_mem:.2f}GB", flush=True)

    # Summary (skip warmup step 1)
    warm_steps = results[1:]
    avg_step = sum(r['step_time'] for r in warm_steps) / len(warm_steps)
    avg_rollout = sum(r['rollout_time'] for r in warm_steps) / len(warm_steps)
    avg_logprob = sum(r['logprob_time'] for r in warm_steps) / len(warm_steps)
    avg_train = sum(r['train_time'] for r in warm_steps) / len(warm_steps)
    avg_mem = sum(r['peak_mem_gb'] for r in warm_steps) / len(warm_steps)

    print(f"\n=== SUMMARY: {mode_str} ===", flush=True)
    ...
    print(f"Avg mem:      {avg_mem:.1f}GB", flush=True)

    output_file = f"/home/zxw/rollout-prefix/pg_v8_results_{'pg_on_flash' if USE_PG and USE_FLASH else 'pg_on_sdpa' if USE_PG else 'pg_off_flash' if USE_FLASH else 'pg_off_sdpa'}_N{N}_P{P_LEN}.json"
