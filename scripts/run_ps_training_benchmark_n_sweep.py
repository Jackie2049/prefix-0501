#!/usr/bin/env python3
"""PS training benchmark: n-sweep comparing model.forward() vs PS ON two-pass.

Scans n=2,4,8 to find the crossover point where PS ON becomes faster than PS OFF.

Key insight from previous benchmark (n=2):
  - Manual forward overhead = 1.01x (negligible)
  - PS ON was 0.70x (slower) because prefix pass (16 layers for 1 seq) adds
    fixed overhead that outweighs n=2 token savings (25%)

This scan tests whether larger n (more token savings) makes PS ON faster.

Usage: torchrun --nproc_per_node=4 scripts/run_ps_training_benchmark_n_sweep.py
"""
...
if local_rank == 0:
    ...
    print(f"Config: prefix={PREFIX_LEN}, suffix={SUFFIX_LEN}")
...
# ===== PS ON two-pass forward =====
def ps_on_forward(n_seq, suffix_ids_lists):
    """PS ON manual two-pass: prefix pass (no_grad, 1 seq) + suffix pass (with_grad, N seqs)."""
...
# ===== Precision validation (n=4) =====
if local_rank == 0:
    print(f"\n{'='*60}")
    print("PRECISION VALIDATION (n=4)")
    print(f"{'='*60}")

n_precision = 4
torch.manual_seed(SEED)
suffix_ids_prec = [torch.randint(0, vocab_size, (SUFFIX_LEN,), device="cpu").tolist()
                   for _ in range(n_precision)]
input_ids_normal = torch.tensor(
    [prefix_ids_list + s for s in suffix_ids_prec],
    dtype=torch.long, device=device)

with torch.no_grad():
    output_normal = model(
        input_ids=input_ids_normal,
        attention_mask=torch.ones((n_precision, total_len), dtype=torch.long, device=device),
        position_ids=torch.arange(total_len, dtype=torch.long, device=device).unsqueeze(0).expand(n_precision, -1))
    logits_normal = output_normal.logits

with torch.no_grad():
    logits_ps = ps_on_forward(n_precision, suffix_ids_prec)
...
for n_val in N_VALUES:
    if local_rank == 0:
        print(f"\n{'='*60}")
        print(f"N = {n_val} | prefix={PREFIX_LEN}, suffix={SUFFIX_LEN}")
        token_savings = (n_val - 1) / n_val * PREFIX_LEN / total_len
        print(f"Token savings: {token_savings:.1%}")
        print(f"{'='*60}")

    # --- PS OFF: model.forward() ---
    if local_rank == 0:
        print(f"  [OFF] PS OFF model.forward() — n={n_val}")
    ...
    for step in range(NUM_WARMUP + NUM_STEPS):
        torch.manual_seed(SEED + step + n_val * 100)
        ...
        t0 = time.time()
        output = model(ids,
            attention_mask=torch.ones(n_val, total_len, dtype=torch.long, device=device),
            position_ids=torch.arange(total_len, dtype=torch.long, device=device).unsqueeze(0).expand(n_val, -1))
    ...
    # --- PS ON: two-pass manual forward ---
    if local_rank == 0:
        print(f"  [ON]  PS ON two-pass manual — n={n_val}")
    ...
    for step in range(NUM_WARMUP + NUM_STEPS):
        ...
        torch.cuda.synchronize()
        t0 = time.time()
        logits = ps_on_forward(n_val, suffix_lists)
        ...
        if step >= NUM_WARMUP:
            times_on.append(t1 - t0)
        if local_rank == 0 and step % 2 == 0:
            print(f"    step {step}: {t1-t0:.3f}s loss={loss.item():.4f} mem={torch.cuda.memory_allocated()/1024**3:.2f}GB")

    avg_on = sum(times_on) / len(times_on)
    peak_on = torch.cuda.max_memory_allocated() / 1024**3
    ...
    result = {
        ...
        "on_avg_s": avg_on,
        "speedup": speedup,
        "peak_mem_off_gb": peak_off,
        "peak_mem_on_gb": peak_on,
    }
    all_results.append(result)

    if local_rank == 0:
        print(f"\n  N={n_val} Results:")
        print(f"    PS OFF: {avg_off:.3f}s/step, peak {peak_off:.2f}GB")
        print(f"    PS ON:  {avg_on:.3f}s/step, peak {peak_on:.2f}GB")
        print(f"    Speedup: {speedup:.2f}x (theoretical: {theoretical:.2f}x)")

# ===== Final summary =====
if local_rank == 0:
    print(f"\n{'='*60}")
    print("N-SWEEP RESULTS SUMMARY")
    print(f"{'='*60}")
    print(f"Model: Qwen3.6-27B-16layers (TP={TP_SIZE}, bf16)")
    print(f"Prefix={PREFIX_LEN}, Suffix={SUFFIX_LEN}")
    print(f"Precision: cos_sim={overall_cos:.6f}")
    print(f"{'='*60}")
    print(f"| n  | Token Savings | Theoretical | OFF (s) | ON (s) | Speedup | Peak OFF | Peak ON |")
    print(f"|----|---------------|-------------|---------|--------|---------|----------|---------|")
    for r in all_results:
        print(f"| {r['n']}  | {r['token_savings_pct']:.1f}%          | {r['theoretical_speedup']:.2f}x        | {r['off_avg_s']:.3f}   | {r['on_avg_s']:.3f}  | {r['speedup']:.2f}x    | {r['peak_mem_off_gb']:.2f}GB   | {r['peak_mem_on_gb']:.2f}GB  |")
    print(f"{'='*60}")

    # Save results
    result_file = os.path.expanduser("~/rollout-prefix/ps_training_n_sweep_results.json")
    with open(result_file, "w") as f:
        json.dump({"results": all_results, "precision": overall_cos, "model": "Qwen3.6-27B-16layers", "tp_size": TP_SIZE}, f, indent=2)
