#!/usr/bin/env python3
"""PS time breakdown: prefix pass vs suffix pass timing.

Usage: torchrun --nproc_per_node=4 scripts/benchmark_ps_time_breakdown.py
"""
import os, sys, time
...
if local_rank == 0:
    print(f"\n{'='*70}")
    print(f"PS TIME BREAKDOWN: 16-layer ~7B, P={PREFIX_LEN}, S={SUFFIX_LEN}")
    print(f"{'='*70}")
...
for N in [4, 8, 16]:
    ...
    if local_rank == 0:
        print(f"\nN={N}: Normal={t_normal:.3f}s, PS_total={t_ps:.3f}s ({t_normal/t_ps:.2f}x)")
        print(f"  Prefix pass: {t_prefix:.3f}s")
        print(f"  Suffix pass: {t_suffix:.3f}s (PS context)")
        print(f"  Token savings: {savings_pct:.1f}%")
        print(f"  Prefix/suffix ratio: {t_prefix/t_suffix:.2f}")
        print(f"  Prefix/normal ratio: {t_prefix/t_normal:.2f}")
...
if local_rank == 0:
    print(f"\n{'='*70}")
    print("Analysis: prefix pass dominates PS overhead")
    print("  - 1×64 tokens runs memory-bound (low GPU utilization)")
    print("  - Solution: increase suffix_len or N to amortize prefix cost")
