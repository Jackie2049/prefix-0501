#!/usr/bin/env python3
"""Multi-step GRPO PS training: simulates multiple optimization steps.

Tests:
1. Multiple training steps with PS (weight updates accumulate)
2. Gradient checkpointing compatibility
3. Multi-step precision degradation check
4. Throughput measurement

Usage: torchrun --nproc_per_node=4 scripts/test_grpo_ps_multistep.py
"""
import os, sys, time
...
# Load pretrained weights (only layers 0-15 + shared weights)
hf_state_dict = {}
for i in range(1, 12):
...
if local_rank == 0:
    ...
    print(f"Loaded {loaded_count} weights, GPU {mem:.2f} GB, num_layers={num_layers}")
...
# Simple SGD optimizer (for testing multi-step training)
optimizer = torch.optim.SGD(model.parameters(), lr=1e-5)

vocab_size = config.vocab_size
total_len = PREFIX_LEN + SUFFIX_LEN
response_len = SUFFIX_LEN - 1  # next-token prediction

clip_ratio = 0.2

if local_rank == 0:
    print(f"\n=== Multi-step GRPO PS Training (16-layer, N={N_SEQUENCES}, P={PREFIX_LEN}, S={SUFFIX_LEN}) ===")
    print(f"Steps: {N_STEPS}, LR: 1e-5, Clip ratio: {clip_ratio}")

step_results = []
for step in range(N_STEPS):
    torch.manual_seed(SEED + step * 100)

    # Create GRPO-like sequences (same prefix for all, different suffixes)
    ...
    # Normal forward (reference, no grad)
    ...
    with torch.no_grad():
        ...
        logits_normal = output_normal.logits.clone()
        log_probs_normal_all = F.log_softmax(logits_normal.float(), dim=-1).clone()

    del output_normal, logits_normal; torch.cuda.empty_cache()

    suffix_log_probs_normal = log_probs_normal_all[:, PREFIX_LEN:, :]
    ...
    del log_probs_normal_all, suffix_log_probs_normal; torch.cuda.empty_cache()
    ...
    torch.cuda.synchronize(); t_forward = time.time() - t_start

    # Compute log_probs and PPO clipped loss
    ...
    if local_rank == 0:
        print(f"Step {step}: loss={policy_loss.item():.4f}, cos_sim={cos_sim:.6f}, "
              f"grad_norm={total_grad_norm:.4f}, fwd={t_forward:.3f}s, total={t_total:.3f}s")

    step_results.append((step, policy_loss.item(), cos_sim, total_grad_norm, t_forward, t_total))
...
if local_rank == 0:
    print(f"\n{'='*60}")
    print(f"MULTI-STEP GRPO PS TRAINING RESULTS (16-layer ~7B)")
    print(f"{'='*60}")
    print(f"{'Step':>4} {'Loss':>8} {'Cos_sim':>10} {'Grad_norm':>10} {'Fwd_t':>8} {'Total_t':>8}")
    for step, loss, cos, gn, ft, tt in step_results:
        print(f"{step:>4} {loss:>8.4f} {cos:>10.6f} {gn:>10.4f} {ft:>8.3f}s {tt:>8.3f}s")
    print(f"{'='*60}")

    # Check: cos_sim should remain > 0.99 across steps
    all_cos_sims = [cos for _, _, cos, _, _, _ in step_results]
    min_cos = min(all_cos_sims)
    if min_cos >= 0.99:
        print(f"PASS: All steps cos_sim >= 0.99 (min={min_cos:.6f})")
    else:
        print(f"FAIL: Some step cos_sim < 0.99 (min={min_cos:.6f})")

    # Check: loss should change across steps (model is being updated)
    losses = [loss for _, loss, _, _, _, _ in step_results]
    loss_changed = any(abs(l1 - l2) > 1e-6 for l1, l2 in zip(losses[:-1], losses[1:]))
    if loss_changed:
        print(f"PASS: Loss changes across steps (learning is happening)")
    else:
        print(f"WARN: Loss barely changes (may need different LR or data)")
