#!/usr/bin/env python3
"""PS scaling benchmark: measuring time only (no precision check) for N=4,8,16.

Usage: torchrun --nproc_per_node=4 scripts/benchmark_ps_n_time.py
"""
import os, sys, time
...
if local_rank == 0:
    print(f"\n{'='*70}")
    print(f"PS TIME BENCHMARK: P={PREFIX_LEN}, S={SUFFIX_LEN}, total={total_len}")
    print(f"{'='*70}")
    print(f"{'N':>4} {'Norm_t':>8} {'PS_t':>8} {'Ratio':>7} {'Save%':>7}")
    print(f"{'-'*4} {'-'*8} {'-'*8} {'-'*7} {'-'*7}")

for N in [4, 8, 16]:
    torch.manual_seed(SEED + 500 + N)

    # Create sequences (same prefix for all)
        ...
        with prefix_sharing_runtime_context(ps_runtime_state) as ctx_state:
            ...
            with torch.no_grad():
                out2 = model(input_ids=suffix_input_ids,
                            ...
                            position_ids=suffix_position_ids)
            del out2
        torch.cuda.synchronize(); t_ps_total += time.time() - t0
    ...
    if local_rank == 0:
        print(f"{N:>4} {t_normal:>8.3f}s {t_ps:>8.3f}s {ratio:>7.2f}x {savings_pct:>6.1f}%")
