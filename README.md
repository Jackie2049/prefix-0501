...
export PREFIX_SHARING_BACKEND=flash_atten_npu   # NPU 推荐 flash_atten_npu（或 torch_ref）
...
export HYDRA_FULL_ERROR=1
export ENABLE_PREFIX_SHARING=1          # 启用 prefix sharing
export PREFIX_SHARING_BACKEND=flash_atten_gpu   # GPU 可选 flash_atten_gpu 加速
...
最简单的开启方式是通过环境变量，无需修改启动命令：

```bash
export ENABLE_PREFIX_SHARING=1                    # 启用功能（必须）
# 根据硬件选择后端：
export PREFIX_SHARING_BACKEND=flash_atten_npu    # NPU 推荐
# export PREFIX_SHARING_BACKEND=flash_atten_gpu  # GPU 推荐
# export PREFIX_SHARING_BACKEND=torch_ref        # 纯 PyTorch（兼容性好，性能稍低）
```

### 当前功能约束

- 支持 TP、物理 PP、Megatron TP-bound SP；不支持 CP、virtual PP
- verl080 monkey patch 同时支持 THD 与 BSHD：`use_remove_padding=True` 走 THD，`use_remove_padding=False` 走 BSHD；Qwen3.5 NPU RL 训练只适配 `use_remove_padding=False`
...
| `flash_atten_npu` | NPU | 基于 MindSpeed/CANN 的 NPU 后端；THD 走融合 attention，BSHD 路径保留 layout-aware 兼容入口 |

如需在 GPU 上使用 flash attention 加速 prefix-sharing 路径，有两种配置方式：

**方式一：环境变量（推荐，全局生效）**

```bash
export PREFIX_SHARING_BACKEND=flash_atten_gpu   # 可选：torch_ref、flash_atten_gpu、flash_atten_npu
```
...
> **优先级说明**：环境变量仅在默认 backend 为 `torch_ref` 时生效；若启动命令中显式设置了 backend，则命令行优先。
>
> **注意**：`flash_atten_gpu` 后端优先服务 THD（varlen）格式，建议配合 `use_remove_padding=True`。Qwen3.5 NPU 当前业务主路径为 BSHD，建议先使用 `torch_ref` 或 `flash_atten_npu` 并在真机上验证。
...
BSHD/THD 不通过 prefix-sharing 自己新增配置选择，而是跟随 verl/Megatron 的 remove-padding 配置：
...
[PS][prepare][global_rank=0 tp_rank=0/tp_size=2 cp_rank=0/cp_size=1] batch_runtime_layout: valid_lengths=..., padded_lengths=..., cu_seqlens=...
[PS][prepare][global_rank=1 tp_rank=1/tp_size=2 cp_rank=0/cp_size=1] batch_runtime_layout: valid_lengths=..., padded_lengths=..., cu_seqlens=...
