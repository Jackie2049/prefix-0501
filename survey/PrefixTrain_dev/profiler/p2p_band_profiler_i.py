...
def run(local_rank, global_rank):
    ...
    for i in range(11):
        ...
        if global_rank == 0:
            for i in range(warmup_times): 
                dist.isend(tensor, dst=1-global_rank)
            send_time_list = []
            for i in range(repeat_times): 
                ...
                send_time_list.append((end - start)*1000)    

            for i in range(warmup_times): 
                dist.irecv(tensor, src=1-global_rank)
            ...
            for i in range(repeat_times): 
                ...
                recv_time_list.append((end - start)*1000)   
        ...
        avg_send_time = sum(send_time_list)/repeat_times
        avg_recv_time = sum(recv_time_list)/repeat_times
        send_bandwidth = (data_size_in_mb/1024) / (avg_send_time/1000)
        recv_bandwidth = (data_size_in_mb/1024) / (avg_recv_time/1000)
        all_send_bandwidths.append(send_bandwidth)
        all_recv_bandwidths.append(recv_bandwidth)
        result_string = f'Rank {global_rank} | Send Time(averaged {repeat_times} times) = {avg_send_time:.2f} ms, Recv Time(averaged {repeat_times} times) = {avg_recv_time:.2f} ms, data_size = {data_size_in_mb:.2f} MB, Send bandwidth = {send_bandwidth:.2f} GB/s, Recv bandwidth = {recv_bandwidth:.2f} GB/s'
    ...
    if global_rank == 0:
        with open(result_file_name, "a+") as f:
            ...
            f_csv.writerow(all_send_bandwidths)
            f_csv.writerow(all_recv_bandwidths)
