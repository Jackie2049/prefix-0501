...
NODE_RANK=0
MASTER_ADDR=192.168.0.16
MASTER_PORT=7111
DISTRIBUTED_ARGS="--nproc_per_node $GPUS_PER_NODE --nnodes $NNODES --node_rank $NODE_RANK --master_addr $MASTER_ADDR --master_port $MASTER_PORT"

#### Paths ####
RESULT_PATH=$ROOT_PATH/logs-large/aceso_${model_size}_gpu_4/block/
...
# if [ "$file_name" != "gpt1_3B_n2_g2.json" ]; then
#     continue
# fi
if [ "$file_name" != "gpt_350M_diff-order-balance_v1_comm_revised_v2_recom_revised_v2_2000_2024-12-11-2-57-53.json" ]; then
    continue
fi
...
prefix_name='3090x2_3090x2_op_coarse_grained'


NCCL_SOCKET_IFNAME=eno12399np0 python3 -m torch.distributed.launch $DISTRIBUTED_ARGS \
       pretrain_gpt.py \
       ...
       --fp16 \
       --log-path $LOG_PATH \
       2>&1 | tee ${LOG_PATH}${prefix_name}_full_log_${config_name}_rank${NODE_RANK}_${CURRENT_TIME}  
