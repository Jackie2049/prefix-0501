...
#### Paths ####
RESULT_PATH=$ROOT_PATH/logs-large/our_test/
...
if [ "$file_name" != "n2_g2.json" ]; then
    continue
fi
NCCL_SOCKET_IFNAME=eno12399np0 python -m torch.distributed.launch $DISTRIBUTED_ARGS \
       pretrain_resnet.py \
