...
def forward_step(forward_step_func, data_iterator, model, input_tensor, losses_reduced, extra_tensors = None, current_data_idx = None, iteration=None):
    ...
    memory.record("forward")
    output_tensor, output_extra_tensors, loss_func = forward_step_func(data_iterator, model, extra_tensors,current_data_idx,iteration)
...
def forward_backward_pipelining_without_interleaving(forward_step_func, data_iterator,
    ...
    # num_microbatches = len(args.mbs_seq[args.current_pipe])
    num_microbatches = args.nbs_each_iter[iteration]
    num_warmup_microbatches = \
    ...
    num_microbatches_remaining = \
        num_microbatches - num_warmup_microbatches
    print(f"rank {torch.distributed.get_rank()} num_microbatches: {num_microbatches}, num_warmup_microbatches: {num_warmup_microbatches}, num_microbatches_remaining: {num_microbatches_remaining}")
    input_tensors = []
    output_tensors = []
    losses_reduced = []
    ...
    for i in range(num_warmup_microbatches):  
        # print(f"rank {torch.distributed.get_rank()} receiving forward tensor idx {recv_forward_idx}")
        input_tensor, extra_tensors = p2p_communication.recv_forward(timers,False,recv_forward_idx)
        recv_forward_idx+=1
        output_tensor, output_extra_tensors = forward_step(forward_step_func, data_iterator, model,
                                     input_tensor, losses_reduced, extra_tensors,current_data_idx,iteration)
        # print(f"rank {torch.distributed.get_rank()} sending forward tensor idx {recv_forward_idx-1}")
        p2p_communication.send_forward(output_tensor, output_extra_tensors, timers,False)
        current_data_idx+=1
        if not forward_only: 
    ...
    for i in range(num_microbatches_remaining):      
        ...
        if forward_only:
            ...
            output_tensor_grad, output_extra_tensors_grad = \
                p2p_communication.send_forward_recv_backward(output_tensor, output_extra_tensors, timers,False,recv_backward_idx)
            recv_backward_idx +=1
        ...
        if forward_only:
            ...
            if last_iteration:
                input_tensor = None
                p2p_communication.send_backward(input_tensor_grad, extra_tensors_grad, timers ,False)
            else:            
                input_tensor, extra_tensors = \
                    p2p_communication.send_backward_recv_forward(
                        input_tensor_grad, extra_tensors_grad, timers,False, recv_forward_idx)
                recv_forward_idx +=1
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):        
            ...
            output_tensor_grad, output_extra_tensors_grad = p2p_communication.recv_backward(timers,False,recv_backward_idx)
            recv_backward_idx+=1
            input_tensor_grad, extra_tensors_grad = \
                backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad, extra_tensors, output_extra_tensors, 
                              output_extra_tensors_grad, model)
...
def forward_backward_pipelining_without_interleaving_simi(forward_step_func, data_iterator,
                                                     model, optimizer, timers,
                                                     forward_only):
    """Run non-interleaved 1F1B schedule, with communication between pipeline
    stages.
    ...
    # Run warmup forward passes.
    for i in range(num_warmup_microbatches):  
        input_tensor, extra_tensors = p2p_communication.recv_forward(timers,True,recv_forward_idx)
        recv_forward_idx+=1
        output_tensor, output_extra_tensors = forward_step(forward_step_func, data_iterator, model,
                                     input_tensor, losses_reduced, extra_tensors,current_data_idx)
        p2p_communication.send_forward(output_tensor, output_extra_tensors, timers,True)
        current_data_idx+=1
        if not forward_only: 
            input_tensors.append(input_tensor)
            output_tensors.append(output_tensor)
    ...
    for i in range(num_microbatches_remaining):      
        ...
        if forward_only:
            ...
            output_tensor_grad, output_extra_tensors_grad = \
                p2p_communication.send_forward_recv_backward(output_tensor, output_extra_tensors, timers,True,recv_backward_idx)
            recv_backward_idx +=1
        ...
        if forward_only:
            ...
            if last_iteration:
                input_tensor = None
                p2p_communication.send_backward(input_tensor_grad, extra_tensors_grad, timers ,True)
            else:            
                input_tensor, extra_tensors = \
                    p2p_communication.send_backward_recv_forward(
                        input_tensor_grad, extra_tensors_grad, timers,True, recv_forward_idx)
                recv_forward_idx +=1
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):        
            ...
            output_tensor_grad, output_extra_tensors_grad = p2p_communication.recv_backward(timers,True,recv_backward_idx)
            recv_backward_idx+=1
            input_tensor_grad, extra_tensors_grad = \
                backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad, extra_tensors, output_extra_tensors, 
                              output_extra_tensors_grad, model)
