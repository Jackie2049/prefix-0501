...
def get_memory_record():
    ...
    _ensure_var_is_initialized(_GLOBAL_MEMORY_RECORD, 'memory record')
    return _GLOBAL_MEMORY_RECORD
...
def _set_memory_record():
    ...
    global _GLOBAL_MEMORY_RECORD
    _ensure_var_is_not_initialized(_GLOBAL_MEMORY_RECORD, 'memory record')
    _GLOBAL_MEMORY_RECORD = MemoryRecord()
...
class _Timer:
    ...
    def start(self):
        ...
        if PROFILING:
            ...
            self.start_time = time.time()
            self.started_ = True
            self.record_time(self.start_time,True)
    ...
    def stop(self):
        ...
        if PROFILING:
            ...
            torch.cuda.synchronize()
            end_time = time.time()
            self.elapsed_ += (end_time - self.start_time)
            self.started_ = False
            self.record_time(end_time,False)
    def start_v2(self):
        """Start the timer."""
        assert not self.started_, f"{self.name_} timer has already been started"
        torch.cuda.synchronize()
    ...
    def record_time(self, time,start=True):
        if start:
            self.start_time_list.append(time)
        else:
...
class Timers:
    ...
    def __init__(self):
        self.timers = {}
        self.time_to_csv =[["iteration","name", "index", "start_time", "end_time","time"]]
        self.warm = True
    ...
    def save_record_time(self, names,iteration):

        if(self.warm):
            for name in names:
                self.timers[name].start_time_list = []
                self.timers[name].end_time_list = []
            return
        for name in names:
            start_time_list = self.timers[name].start_time_list
            end_time_list = self.timers[name].end_time_list
            for i in range(len(start_time_list)):
                self.time_to_csv.append([iteration,name, i, start_time_list[i], end_time_list[i], end_time_list[i] - start_time_list[i]])
            self.timers[name].start_time_list = []
            self.timers[name].end_time_list = []
    
    def clear_record_time(self, names):
        for name in names:
            self.timers[name].start_time_list = []
            self.timers[name].end_time_list = []

    def get_record_time(self):
        return self.time_to_csv
...
class MemoryRecord():
    ...
    def record(self, tag):
        ...
        self.records.append({
            ...
            "inactive_MB": (reserved - allocated) / 1024**2,
            "timestamp": time.time()
        })
    ...
    def record_activation(self, tag, key ,value):
        if(self.warm):
            return
        torch.cuda.synchronize()
        allocated = torch.cuda.memory_allocated()
