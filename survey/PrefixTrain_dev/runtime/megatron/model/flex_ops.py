...
from megatron.model.utils import init_method_normal
from megatron.context_parallel.ring_context_parallel import ringattn_context_parallel
from megatron.context_parallel.ulysses_context_parallel import UlyssesContextAttention, UlyssesContextAttention_kvpack
from  flash_attn import flash_attn_func, flash_attn_varlen_kvpacked_func
from megatron.ring_flash_attn import ring_flash_attn_varlen_kvpacked_func,zigzag_ring_flash_attn_varlen_kvpacked_func,stripe_flash_attn_kvpacked_func
from megatron.memory_manager.memory import MemoryManager
...
def bias_dropout_add_npu(x, bias, residual, prob, training):
    # type: (Tensor, Tensor, Tensor, float, bool) -> Tensor
    # print("bias_dropout_add_npu")
    out, mask = torch_npu._npu_dropout(x + bias, prob)
    out = residual + out
    return out
...
def gen_op(op_info, algo):
    if "conv" in op_info["name"]:
        ...
        if("v1" in op_info["name"]):
               op = Embedding_v1(name=op_info["name"], 
                        op_index = op_info["op_index"], 
        ...
        if("v1" in op_info["name"]):
            op = ParallelLayerNormOp_v1(name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])
        else:
        ...
        if("v1" in op_info["name"]):
            op = ParallelQKVOp_v1(name=op_info["name"], 
                                op_index = op_info["op_index"], 
                                init_method = op_info["init_method"],
        ...
        if("v1" in op_info["name"]):
            op = ParallelDropoutOp_v1(name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])   
        else:
        ...
        op = T5PostProcess(name=op_info["name"], 
                                 ...
                                 init_method = op_info["init_method"],
                                 prev_name=op_info["prev_name"])
    elif "enc-flash-attention" in op_info["name"]:
        if(get_args().context_parallel_algo == "megatron_cp_algo"):
            op = RingFlashAtten (name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])
        elif(get_args().context_parallel_algo == "ulysses_cp_algo"):
            if(get_args().packing is True):
                op = UlyssesFlashAtten_kvpack (name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])
            else:
                op = UlyssesFlashAtten (name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])
        else:
            op = ParallelFlashAtten(name=op_info["name"], 
                                    op_index = op_info["op_index"], 
                                    prev_name=op_info["prev_name"])
    else:
...
class Embedding(RopalaModule):
    ...
    def __init__(self, op_index,
        ...
        if self.name == "encoder-embedding":
            ...
            self.input_tensors_info = {"enc_input_ids": {"shape": [args.micro_batch_size, args.seq_length], "tp_split_dim": -1, "dp_split_dim": -1, "cp_split_dim":1 },
                                        "enc_position_ids": {"shape": [args.micro_batch_size, args.seq_length], "tp_split_dim": -1, "dp_split_dim": -1 , "cp_split_dim":1}}
...
class Embedding_v1(RopalaModule):
    """Language model embeddings.
    ...
    def __init__(self, op_index,
                 ...
                 num_tokentypes,
                 name=None, prev_name=None):
        super(Embedding_v1, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.hidden_size = args.hidden_size
        self.init_method = init_method
        self.num_tokentypes = num_tokentypes
        ...
        ## tensor shape information
        if self.name == "encoder-embedding-v1":
            # self.input_tensors_info = {"input_tensor": {"shape": [], "tp_split_dim": -1, "dp_split_dim": -1}}
            self.input_tensors_info = {"enc_input_ids": {"shape": [args.micro_batch_size, args.seq_length], "tp_split_dim": -1, "dp_split_dim": -1, "cp_split_dim":1 },
                                        "enc_position_ids": {"shape": [args.micro_batch_size, args.seq_length], "tp_split_dim": -1, "dp_split_dim": -1, "cp_split_dim":1 }}
            self.output_tensors_info = {"hidden_states": {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}}
            if args.model_name == "gpt":
                self.shared_weights_info = {"word_embeddings": {"root": True, "sharing_with_ops": [10*args.num_layers +2], "shape": [args.padded_vocab_size, args.hidden_size], "tp_split_dim": 0, "dp_split_dim": -1}}
            elif args.model_name == "t5":
                self.shared_weights_info = {"word_embeddings": {"root": True, "sharing_with_ops": [args.num_layers*10+2, args.num_layers*(10+21)+4], "shape": [args.padded_vocab_size, args.hidden_size], "tp_split_dim": 0, "dp_split_dim": -1},
                                            "position_embeddings": {"root": True, "sharing_with_ops": [args.num_layers*10+2], "shape": [args.max_position_embeddings, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": -1}}
        elif self.name == "decoder-embedding":
            self.input_tensors_info = {"encoder_output": {"shape": [args.micro_batch_size, args.seq_length, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 0}}
            self.output_tensors_info = {"hidden_states": {"shape": [args.decoder_seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1},
                                        "encoder_output": {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1}}
...
class ParallelLayerNormOp(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        ...
        # print(self.name ,extra_tensor_name)
        # if "enc" in self.name:
        ...
        ## tensor shape information
        self.input_tensors_info = {"hidden_states": {"shape": list(hidden_states_size), "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim": 0 }}
        self.output_tensors_info = {"hidden_states": {"shape": list(hidden_states_size), "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim": 0 }}
        
        if args.model_name == "gpt" and self.name == "final-layernorm":
            self.output_tensors_info = {"hidden_states": {"shape": [args.micro_batch_size, args.seq_length, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 0, "cp_split_dim":1}}
        elif args.model_name == "t5" and self.name == "enc-final-layernorm":
            self.output_tensors_info = {"encoder_output": {"shape": [args.micro_batch_size, args.seq_length, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 0, "cp_split_dim":1}}
        ...
        if extra_tensor_name is not None:
            self.output_extra_tensors_info = {extra_tensor_name: {"shape": list(hidden_states_size), "tp_split_dim": -1, "dp_split_dim": 1, "send_to": extra_tensor_send_to, "cp_split_dim": 0}}
        ## tensor resharding information
...
class ParallelLayerNormOp_v1(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(ParallelLayerNormOp_v1, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.input_layernorm = LayerNorm(
            args.hidden_size,
            eps=args.layernorm_epsilon)

        if self.name in ["enc-1st-layernorm-v1", "dec-1st-layernorm"]:
            extra_tensor_name = "hidden_states_1"
            extra_tensor_send_to = 4
        elif self.name == "enc-2nd-layernorm-v1":
            extra_tensor_name = "hidden_states_2"
            extra_tensor_send_to = 4
        elif self.name == "dec-2nd-layernorm":
            extra_tensor_name = "hidden_states_2"
...
class ParallelQKVOp(RopalaModule):
    def __init__(self, init_method,
        ...
        if self.name == "enc-attention-qkv":
            ...
            self.input_tensors_info = {"hidden_states": {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_tensors_info = {"query_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0},
                                        "key_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_extra_tensors_info = {extra_tensor_name: {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "send_to": 4, "cp_split_dim": 0}} 
...
class ParallelQKVOp_v1(RopalaModule):
    def __init__(self, init_method,
                 attention_type, op_index, algo=0, name=None, prev_name=None):
        super(ParallelQKVOp_v1, self).__init__(op_index, name, prev_name)
        args = get_args()
        ...
        if self.algo == "row":
            assert self.resharding is True        
        ## shape information
        if self.name == "enc-attention-qkv-v1":
            extra_tensor_name = "value_layer"
            self.input_tensors_info = {"hidden_states": {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1 ,"cp_split_dim":0}}
            self.output_tensors_info = {"query_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1,"cp_split_dim":0},
                                        "key_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1,"cp_split_dim":0}}
            self.output_extra_tensors_info = {extra_tensor_name: {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "send_to": 1,"cp_split_dim":0}} 
        elif self.name == "dec-attention-qkv-1":
            extra_tensor_name = "value_layer_1"
            self.input_tensors_info = {"hidden_states": {"shape": [args.decoder_seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1},
                                        "encoder_output": {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1}}
...
class ParallelSoftmaxFusionOp(RopalaModule):
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False):
        ...
        seq_len = int(hidden_states.size()[3])
        if self.name == "enc-attention-softmax":          
            shape_0 = input_extra_tensors["enc_attention_mask"].shape[0]
            attention_mask = input_extra_tensors["enc_attention_mask"][0:shape_0//self.dp_size, :, :seq_len, :seq_len]
        elif self.name == "dec-attention-softmax-1":
            attention_mask = input_extra_tensors["dec_attention_mask"][0:micro_batch_size, :, :seq_len, :seq_len]  
        elif self.name == "dec-attention-softmax-2":
            attention_mask = input_extra_tensors["enc_dec_attention_mask"][0:micro_batch_size, :, :seq_len, :seq_len] 

        # ============  ===============
        # Attention probs and dropout
        # ===========================
        attention_mask  = torch.rand((1, 1, seq_len, seq_len), requires_grad=False, device=torch.cuda.current_device()) < 0.5
...
class ParallelFlashAtten(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(ParallelFlashAtten, self).__init__(op_index, name, prev_name)
        args = get_args()

        ## shape info
        if self.name == "enc-flash-attention":
            extra_tensor_name = "value_layer"
            self.input_tensors_info = {"query_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0},
                                        "key_layer": {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_tensors_info = {"hidden_states": {"shape": [args.seq_length, args.micro_batch_size, args.kv_channels * args.num_attention_heads], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
            self.input_extra_tensors_info = {extra_tensor_name: {"shape": [args.seq_length, args.micro_batch_size, args.num_attention_heads, args.kv_channels], "tp_split_dim": 2, "dp_split_dim": 1, "recv_from": -1, "cp_split_dim":0}}   
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False): 
        ...
        # [sq, b, np, hn] -> [sq, b * np, hn] [b,h, sq*hn]
        query_layer = query_layer.view(query_layer.size(1),query_layer.size(0),  -1)
        
        # [sk, b, np, hn] -> [sk, b * np, hn] [b,sq,h]
        key_layer = key_layer.view(key_layer.size(1),key_layer.size(0),  -1)




        # [s ,b, n, c]   change view [b, s ,h] 
        value_layer = value_layer.view( value_layer.size(1) , value_layer.size(0),  -1)
        # print(query_layer.shape)
        # print(key_layer.shape)
        # print(value_layer.shape)
        # print(output_size)

        attention_score, softmax_max, softmax_sum, softmax_out, seed, offset, numels =torch_npu.npu_fusion_attention(query_layer, key_layer, value_layer, head_num=output_size[1], input_layout="BSH", scale=0.08838)
        
        # [b, s , h]   change view  [s ,b,  n* c] 
        attention_score = attention_score.view(output_size[2] , output_size[0], -1)

        # print(attention_score.shape)

        output_tensors["hidden_states"] = attention_score
...
class RingFlashAtten(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(RingFlashAtten, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.args = args
        ## shape info
        if self.name == "enc-flash-attention":
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False): 
        ...
        query_layer = input_tensors["query_layer"]
        key_layer = input_tensors["key_layer"]
        cache_len = input_extra_tensors.get("enc_cache_len", None)
        q_cu_seq_lens = input_extra_tensors.get("enc_cu_seq_lens", None)
        k_cu_seq_lens = input_extra_tensors.get("enc_k_cu_seq_lens", None)
        max_seqlen_q = input_extra_tensors.get("max_seqlen_q", None)
        max_seqlen_k = input_extra_tensors.get("max_seqlen_k", None)
        query_layer = input_tensors["query_layer"]
        key_layer = input_tensors["key_layer"]
        # print(query_layer.shape)
        # print(key_layer.shape)
        ...
        scale = 1.0 / math.sqrt(query_layer.size(2))
        ...
        if(cp_size > 1):
            ...
            # query_layer 维度为 s,b,n,d ,其中b=1，把他去除
            query_layer = query_layer.view(query_layer.size(0), query_layer.size(2), -1)
            kv_layer = torch.cat([key_layer, value_layer], dim=1)
            # print("query_layer:", query_layer.shape)
            # print("kv_layer:", kv_layer.shape)
            if(self.args.zigzag_ring_attn):
                out_ = zigzag_ring_flash_attn_varlen_kvpacked_func(query_layer, kv_layer, cu_seqlens=q_cu_seq_lens, max_seqlen=max_seqlen_q, causal=True,group=cp_group)
            else: 
                out_ = ring_flash_attn_varlen_kvpacked_func(query_layer, kv_layer, cu_seqlens=q_cu_seq_lens, max_seqlen=max_seqlen_q, causal=True,group=cp_group)
        else:
            out_ = flash_attn_func(query_layer, key_layer, value_layer,causal = True)
        
        out_= out_.view(output_size[2] , output_size[0], -1)
        output_tensors["hidden_states"] = out_
        return output_tensors
...
class UlyssesFlashAtten(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(UlyssesFlashAtten, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.args = args
        self.store_for_sample_idx = args.store_for_sample_idx
        self.shared_for_sample_idx = args.shared_for_sample_idx
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.memory_manager = MemoryManager(op_index,self.store_for_sample_idx,self.shared_for_sample_idx,device,args)
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False): 
        ...
        seq_len , bs, num_head = query_layer.size(0) , query_layer.size(1) ,query_layer.size(2)

        # [sq, b, np, hn] -> [sq, b * np, hn] [b, sq, hn, np]
        query_layer = query_layer.view(seq_len, bs, num_head, -1)
        key_layer = key_layer.view(seq_len, bs, num_head, -1)
        value_layer = value_layer.view(seq_len, bs, num_head, -1)

        

        cp_size = mpu.get_context_parallel_world_size()
        if(cp_size>1):
            raise NotImplementedError("UlyssesContextAttention only support packing=True now")

        else:
            # out_, softmax_max, softmax_sum, softmax_out, seed, offset, numels =torch_npu.npu_fusion_attention(query_layer, key_layer, value_layer, head_num=output_size[1], input_layout="BSH", scale=0.08838)
            #换成 cuda pytorch flash attention
            # bsnh
            out_ = flash_attn_func(query_layer, key_layer, value_layer,causal = True)
...
class UlyssesFlashAtten_kvpack(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(UlyssesFlashAtten_kvpack, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.layer_idx = op_index// args.num_ops_each_layer
        self.args = args
        self.store_for_sample_idx = args.store_for_sample_idx
        self.shared_for_sample_idx = args.shared_for_sample_idx
        self.memory_manager = MemoryManager(self.layer_idx,self.tp_size,self.store_for_sample_idx,self.shared_for_sample_idx,args)
        self.forward_count = 0
        ## shape info
        if self.name == "enc-flash-attention":
            extra_tensor_name = "value_layer"
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False): 
        ...
        next_cu_seq_lens = input_extra_tensors.get("enc_next_cu_seq_lens", None)
        next_k_cu_seq_lens = input_extra_tensors.get("enc_next_k_cu_seq_lens", None)
        next_max_seqlen_q = input_extra_tensors.get("next_max_seqlen_q", None)
        next_max_seqlen_k = input_extra_tensors.get("next_max_seqlen_k", None)
        current_data_idx = input_extra_tensors.get("current_data_idx", None)
            # print("k_cu_seq_lens", k_cu_seq_lens)
        max_seqlen_q = input_extra_tensors.get("max_seqlen_q", None)
        max_seqlen_k = input_extra_tensors.get("max_seqlen_k", None)
        query_layer = input_tensors["query_layer"]
        key_layer = input_tensors["key_layer"]
        system_warm = input_extra_tensors.get("system_warm", False)

        packed_seq_len = query_layer.size(0)
        num_head = query_layer.size(2)
        scale = 1.0 / math.sqrt(query_layer.size(2))
        ...
        if(cp_size>1):
            cp_group = mpu.get_context_parallel_group()
            ulysses_attention = UlyssesContextAttention_kvpack(cp_group)
            out_ = ulysses_attention(query_layer, key_layer, value_layer, cu_seqlens_q=q_cu_seq_lens, cu_seqlens_k=k_cu_seq_lens, max_seqlen_q=max_seqlen_q, max_seqlen_k=max_seqlen_k, causal=True, window_size=(-1, -1),
            alibi_slopes=None,
            deterministic=False,
            return_attn_probs=False)
        else:
            key_vaule_pack = torch.cat([key_layer, value_layer], dim=1)
            query_layer = query_layer.view(packed_seq_len, num_head, -1)
            # print("query_layer:", query_layer.shape)
            # print("key_vaule_pack:", key_vaule_pack.shape)
            if(self.args.share_activation and system_warm!=True):
                # key_vaule_pack= self.memory_manager.retrieve_activation(key_vaule_pack, self.forward_count,q_cu_seq_lens,k_cu_seq_lens)
                key_vaule_pack = self.memory_manager.get_from_prefetch(key_vaule_pack, batch_idx=current_data_idx, q_cu_seq_lens=q_cu_seq_lens, k_cu_seq_lens=k_cu_seq_lens)
            out_ = flash_attn_varlen_kvpacked_func(query_layer, key_vaule_pack, cu_seqlens_q =q_cu_seq_lens, cu_seqlens_k=k_cu_seq_lens, max_seqlen_q=max_seqlen_q, max_seqlen_k=max_seqlen_k, causal=True, window_size=(-1, -1),
    alibi_slopes=None,
    deterministic=False,
    return_attn_probs=False)
            if(self.args.share_activation and system_warm!=True):
                self.memory_manager.store_activation(key_vaule_pack,current_data_idx,k_cu_seq_lens=k_cu_seq_lens)
                self.memory_manager.retrieve_activation_prefetch(batch_idx=current_data_idx+1,q_cu_seq_lens=next_cu_seq_lens,k_cu_seq_lens=next_k_cu_seq_lens,device=query_layer.device, dtype=query_layer.dtype)
                self.memory_manager.clear_activation(current_data_idx)
                self.memory_manager.async_gc()
        


        # [s,h,d] -> [s,b,n*c]
        out_ = out_.view(packed_seq_len, 1, -1)
...
class ParallelGEMM(RopalaModule):
    def __init__(self, op_index, init_method, dim1_size, dim2_size, algo=0, name=None, prev_name=None):
        ...
        if self.algo == "row":
            ## shape information
            self.input_tensors_info = {"hidden_states": {"shape": [seq_len, args.micro_batch_size, dim1_size], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_tensors_info = {"output_tensor": {"shape": [seq_len, args.micro_batch_size, dim2_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_extra_tensors_info = {"output_bias": {"shape": [dim2_size], "tp_split_dim": -1, "dp_split_dim": -1, "send_to": 1, "cp_split_dim":0}}  
            ## resharding information
            self.required_input_specs = {"hidden_states": {"R": 1, "V": 1, "dims": [1, self.dp_size, self.tp_size]}} 
            ...
            ## shape information
            self.input_tensors_info = {"hidden_states": {"shape": [seq_len, args.micro_batch_size, dim1_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_tensors_info = {"output_tensor": {"shape": [seq_len, args.micro_batch_size, dim2_size], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
            self.output_extra_tensors_info = {"output_bias": {"shape": [dim2_size], "tp_split_dim": 0, "dp_split_dim": -1, "send_to": 1, "cp_split_dim": -1}}  #TODO
            ## resharding information            
            self.required_input_specs = {"hidden_states": {"R": self.tp_size, "V": 1, "dims": [1, self.dp_size, 1]} }      
        ...
        if "dec" in self.name:
            self.input_tensors_info["encoder_output"] = {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}
            self.output_tensors_info["encoder_output"] = {"shape": [args.seq_length, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":0}
            self.required_input_specs["encoder_output"] = {}
            self.output_specs["encoder_output"] = {}
            self.output_mats_info["encoder_output"] = {}
        ## profiling
...
class ParallelDropoutOp(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        ...
        ## shape info
        self.input_tensors_info = {"output_tensor": {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1 , "cp_split_dim": 0 }}
        self.output_tensors_info = {"hidden_states": {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim": 0}}
        if input_extra_name is not None:
            self.input_extra_tensors_info = {input_extra_name[0]: {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "recv_from": recv_from, "cp_split_dim":0},
                                             input_extra_name[1]: {"shape": [args.hidden_size], "tp_split_dim": -1, "dp_split_dim": -1, "recv_from": -1, "cp_split_dim": -1}}  
...
class ParallelDropoutOp_v1(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        super(ParallelDropoutOp_v1, self).__init__(op_index, name, prev_name)
        args = get_args()
        self.bias_dropout_fusion = args.bias_dropout_fusion
        self.hidden_dropout = args.hidden_dropout

        if self.name == "enc-post-attention-dropout-v1":
            input_extra_name = ["hidden_states_1", "output_bias"]
            recv_from = -3
        elif self.name == "enc-post-MLP-dropout-v1":
            input_extra_name = ["hidden_states_2", "output_bias"]
            recv_from = -4
        elif self.name == "dec-post-attention-dropout-1":
            input_extra_name = ["hidden_states_1", "output_bias"]
        ...
        ## shape info
        self.input_tensors_info = {"output_tensor": {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":1, "cp_split_dim":0 }}
        self.output_tensors_info = {"hidden_states": {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "cp_split_dim":1, "cp_split_dim":0 }}
        if input_extra_name is not None:
            self.input_extra_tensors_info = {input_extra_name[0]: {"shape": [seq_len, args.micro_batch_size, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 1, "recv_from": recv_from, "cp_split_dim":0},
                                             input_extra_name[1]: {"shape": [args.hidden_size], "tp_split_dim": -1, "dp_split_dim": -1, "recv_from": -1, "cp_split_dim":-1}}  
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False):   
        ...
        if "enc-post-attention-dropout-v1" in self.name or self.name ==  "dec-post-attention-dropout-1":
            with torch.enable_grad():       
                layernorm_input = bias_dropout_add_func(
                    input_tensors["output_tensor"],
                    input_extra_tensors["output_bias"].expand_as(input_extra_tensors["hidden_states_1"]),
        ...
        elif self.name == "enc-post-MLP-dropout-v1" or self.name == "dec-post-attention-dropout-2":
            with torch.enable_grad():       
                layernorm_input = bias_dropout_add_func(
                    input_tensors["output_tensor"],
                    input_extra_tensors["output_bias"].expand_as(input_extra_tensors["hidden_states_2"]),
...
class ParallelMlpGeLUOp(RopalaModule):
    def __init__(self, op_index, name=None, prev_name=None):
        ...
        args = get_args()
        self.bias_gelu_fusion = args.bias_gelu_fusion=False
        ...
        ## shape info
        self.input_tensors_info = {"output_tensor": {"shape": [seq_len, args.micro_batch_size, args.ffn_hidden_size], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
        self.output_tensors_info = {"hidden_states": {"shape": [seq_len, args.micro_batch_size, args.ffn_hidden_size], "tp_split_dim": 2, "dp_split_dim": 1, "cp_split_dim":0}}
        self.input_extra_tensors_info = {"output_bias": {"shape": [args.ffn_hidden_size], "tp_split_dim": 0, "dp_split_dim": -1, "recv_from": -1 , "cp_split_dim":-1}}  
        ## resharding info                     
...
class GPTPostProcess(RopalaModule):
    def __init__(self, parallel_output, init_method, op_index, name=None, prev_name=None):
        ...
        ## shape info
        self.input_tensors_info = {"hidden_states": {"shape": [args.micro_batch_size, args.seq_length, args.hidden_size], "tp_split_dim": -1, "dp_split_dim": 0 ,"cp_split_dim": 1}}            
        self.output_tensors_info = {"output_tensor": {"shape": [1], "tp_split_dim": -1, "dp_split_dim": -1 ,"cp_split_dim": -1}}
        self.shared_weights_info ={"word_embeddings": {"root": False, "sharing_with_ops": [0], "shape": [args.padded_vocab_size, args.hidden_size], "tp_split_dim": 0, "dp_split_dim": -1}}
        ## resharding info
    ...
    def forward(self, input_tensors, input_extra_tensors, output_extra_tensors, profiling=False):
        ...
        args = get_args()
        labels = torch.rand((lm_output.shape[0],lm_output.shape[1] ), requires_grad=False, device=torch.cuda.current_device()).long() * args.padded_vocab_size
