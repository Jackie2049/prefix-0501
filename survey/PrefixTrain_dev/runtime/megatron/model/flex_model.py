...
def initialize_comm_info(tensors_info, dst_stage, src_op_index=0, dst_op_index=0):
    ...
    for key in sorted(tensors_info):
        if key not in ["input_tensor"]:
            ...
            recv_info["tensors"][key] = {"tp_split_dim": tp_split_dim, "num_tp_chunks": num_tp_chunks, "dp_split_dim": dp_split_dim, "num_dp_chunks": num_dp_chunks, "cp_split_dim": cp_split_dim, "num_cp_chunks": num_cp_chunks }
            send_info["tensors"][key] = {"tp_split_dim": tp_split_dim, "num_tp_chunks": num_tp_chunks, "tp_chunks_index": [0], "dp_split_dim": dp_split_dim, "num_dp_chunks": num_dp_chunks, "dp_chunks_index": [0], "cp_split_dim": cp_split_dim, "num_cp_chunks": num_cp_chunks, "cp_chunks_index": [0]}
            ...
            if dst_cp_size < cp_size:
                ...
                if cp_split_dim != -1:
                    ...
                    send_info["tensors"][key]["num_cp_chunks"] = num_cp_chunks
                    send_info["tensors"][key]["cp_chunks_index"] = [cp_id % num_cp_chunks]   
...
class FlexPipeModel(MegatronModule):
    def __init__(self, full_model_op_list, pre_process=True, post_process=True):
        ...
        if(len(self.recompute_ops) != len(full_model_op_list)):
            self.recompute_ops = [0] * len(full_model_op_list)
            print("warning: recompute_ops length mismatch, set to no recompute")
