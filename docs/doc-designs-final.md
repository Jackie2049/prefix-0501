...
`prefix-0501` 暂时以整个项目为版本管理颗粒度，`survey/`、`dependency/`、`docs/`、`prefix-sharing/` 一起管理。`prefix-sharing/` 是当前实际开发目录；文档统一放在 `docs/`；后续可以再独立拆分为开源仓库。

参考项目的定位如下：

- `survey/PrefixTrain_dev`：最重要的设计参考。其在线 Trie 思路支持 provider 面向不同 reuser 提供不同长度的子序列复用。Phase 1 采用同类 per-sample reuse relation，而不是“一个 group 只有一个统一 prefix_len”的简化设计。
...
### 4.6 PrefixKVSlotId / StoredPrefixKV
...
```python
config = PrefixSharingConfig(enable_prefix_sharing=True, min_prefix_len=8)
...
```python
store.store(slot_id, key_tensor=key_tensor, value_tensor=value_tensor, prefix_len=n, overwrite=True)
...
当前项目版本管理以 `prefix-0501` 为颗粒度。`survey/`、`dependency/`、`docs/`、`prefix-sharing/` 暂时一起提交和推送。

`prefix-sharing/` 曾经作为独立 Git 仓库开发。该独立历史已保留在：

- `https://github.com/Jackie2049/prefix-sharing.git`
- 最后独立提交：`af21cd0 [feat] 支持按样本前缀复用关系`

后续开源 `prefix-sharing` 时，可以基于当前 `prefix-0501` 的 `prefix-sharing/` 路径拆分，也可以基于保留的独立仓库历史继续整理。
...
- `core/prefix_store.py`：forward/micro-batch/layer/provider/tp 维度的 K/V store。
- `backends/torch_ref.py`：reference attention backend，支持 q_len != kv_len 和 transitive reuse store。
