...
**PrefixSharingBatchMeta** — 一个 micro-batch 内部单词前向的前缀复用执行计划
```
...
│   │   ├── planner.py                   # PrefixSharingPlanner 执行规划
│   │   ├── metadata.py                  # PrefixSharingBatchMeta 元数据
...
| `core/planner.py` | `PrefixSharingPlanner` | 将检测结果转为执行计划 |
| `core/metadata.py` | `PrefixSharingBatchMeta` | Batch 级执行计划数据结构 |
...
| `integrations/verl_mcore.py` | `megatron_actor_prefix_sharing_context` | Context manager for verl |
| `integrations/verl_mcore.py` | `restore_megatron_actor_log_probs` | verl logprob 恢复 |
...
└────┬────┘     └────────┬─────────┘     └──────────┬──────────┘     └────────┬────────┘
     ...
     │                   │                        │     store.close()        │
     │                   │                        │                         │
     │  16. restore_megatron_actor_log_probs()  │                         │
     │──────────────────────────────────────────>│                         │
...
| 16-18 | `MegatronPPOActor` | `restore_megatron_actor_log_probs` | Prefix-Last Restore，恢复 reuser 的 logprob |
...
```
1. 配置检查
   └── prefix_sharing_config_from_verl(actor_config)
   
2. 前置条件检查（6条路径）
   ├── Path 1: config.enable_prefix_sharing=False → 返回 (batch, None)
   ├── Path 2: use_remove_padding=False → RuntimeError
...
5. 计算 THD 1D 位置
   └── cu_seqlens_cpu (考虑 TP/CP 对齐)
   
6. 构建 restore slots
   └── PackedPackedPrefixLastRestoreSlot (provider/reuse 1D 位置)
   
7. 组装 RuntimeState
   └── PrefixSharingRuntimeState(
         prefix_sharing_plan=plan,
         backend=TorchReferenceBackend(),
         kept_position_ids=...,      # 用于 RoPE
         prefix_last_restore_slots=..., # 用于 logprob 恢复
       )
```
...
| `PrefixSharingRuntimeState` | 跨层传递的运行时状态，包含 plan、backend、位置信息 |
| `PackedPackedPrefixLastRestoreSlot` | THD 格式下的 1D 位置映射，用于 logprob 恢复 |

**restore_megatron_actor_log_probs 逻辑**:

```python
def restore_megatron_actor_log_probs(logits, labels, log_probs, vocab_fn):
    ...
    for spec in ctx.prefix_last_restore_slots:
        ...
        provider_logits = logits[0:1, spec.provider_1d_pos:spec.provider_1d_pos+1, :]
        # 用 reuser 的第一个 suffix label 计算 logprob
        reuse_label = labels[0:1, spec.reuse_1d_pos:spec.reuse_1d_pos+1]
        restored[0, spec.reuse_1d_pos] = vocab_fn(provider_logits, reuse_label)
    return restored
```
...
class PrefixSharingRuntimeContext:
    ...
    kept_position_ids: Any | None = None      # 保留的位置编码
...
def prefix_sharing_runtime_context(runtime_state):
    ctx = PrefixSharingRuntimeContext(
        ...
        kept_position_ids=runtime_state.kept_position_ids,
        prefix_last_restore_slots=runtime_state.prefix_last_restore_slots,
    )
...
def maybe_run_prefix_sharing_attention(
    ...
    assert ctx.kept_position_ids is not None
    
    # 3. 应用 RoPE（使用 kept_position_ids 而非默认位置）
    query, key = _apply_positioned_rope(
        attention_module, query, key,
        q_pos_emb, k_pos_emb,
        ctx.kept_position_ids,  # 关键：原始位置编码
        mscale=mscale
    )
...
```python
def _apply_positioned_rope(..., kept_position_ids, ...):
    positions = kept_position_ids.to(device=query.device)
...
│    5. 调用 restore_megatron_actor_log_probs() 恢复 logprob         │
│                                                                  │
│  attention.py:                                                   │
...
│    - restore: 从 context 读取 slots → 复制 provider logits        │
│                                                                  │
│  context.py:                                                     │
