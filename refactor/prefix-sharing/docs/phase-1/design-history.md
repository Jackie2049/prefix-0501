...
> **规则**: 所有方案设计按时间倒序记录，最新在前。本文档保留历史讨论、被推翻方案、备选方案和阶段性分析；当前用于开发的最终方案见 `design-final.md`。
...
- `design-history.md`：保留历史讨论和阶段性分析，时间倒序，不删除旧结论。
- `design-final.md`：只维护 Phase 1 最终规格，用于后续开发。
